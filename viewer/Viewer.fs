...
// F# viewer — renders sketch geometry, dimensions, and frame gizmos on top
// of a WebGPU canvas. Subscribes to AppStore for state; dispatches picks,
// drags, and tool clicks back.
    ...
    promise {
        ...
        | Some g ->
            ...
            if isNull adapter then
                ...
                let frameAxisPickSlot = makeSlot ()
                ...
                // PickId → Pickable lookup for resolving GPU picks. Rebuilt
                // whenever compilation output changes.
                    ...
                    | Some button ->
                        ...
                        let beginConstraintLabelDrag sid cix u v =
                            BeginSketchDrag
                                { SketchId = sid
                                  ...
                                  Target = { X = u; Y = v } }
                        let updateDragTo u v =
                            UpdateSketchDragTarget { X = u; Y = v }
                        ...
                        | 0, Some (PickPoint(_, sid, pid, _, _)) ->
                            ...
                            if not dragActive && movedPx > DRAG_THRESHOLD_PX then
                                ...
                                match mouseToSketchLocal sid x y with
                                | Some (u, v) ->
                                    Store.dispatch AppStore.store (updateDragTo u v)
                                | None -> ()
                        | 0, Some (PickDimension(_, sid, cix, _)) ->
                            if not dragActive && movedPx > DRAG_THRESHOLD_PX then
                                ...
                                | Some (u, v) ->
                                    dragActive <- true
                                    Store.dispatch AppStore.store
                                        (beginConstraintLabelDrag sid cix u v)
                                | None -> ()
                            elif dragActive then
                                match mouseToSketchLocal sid x y with
                                | Some (u, v) ->
                ...
                let rec frame (_: float) =
                    ...
                    for sketch in model.Sketches do
                        ...
                        | Some transform ->
                            ...
                            let gizmoData = SketchOverlayRender.buildSketchGizmoBuffer ()
                            let (br, cr) = gizmoSlot
                    ...
                    let frameOriginPickData =
                        SketchOverlayRender.buildFrameOriginsPickBuffer
